...
namespace LisaKatherine.Models
{
    public class Utils
    {
        ...
        public static string StripInvalid(string htmlString)
        {
            var retVal = Regex.Replace(htmlString, @"/[^a-zA-Z0-9\s\p{P}]/", string.Empty);
            return retVal.Replace(" ", "_");
        }

        public static string GetSummary(string htmlString, int stringLen)
        {
            return htmlString.Length > stringLen ? String.Format("{0}...", htmlString.Substring(0, stringLen)) : htmlString;
        }
